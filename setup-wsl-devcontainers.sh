...
SERVICE_EOF
  ...
  # Create environment.d file for session-wide SSH_AUTH_SOCK
  # This ensures VS Code Remote and other non-interactive processes can access the socket
  local -r env_dir="${user_home}/.config/environment.d"
  mkdir -p "${env_dir}"
  chown "${username}:${username}" "${env_dir}"

  local -r env_file="${env_dir}/50-ssh-agent.conf"
  cat >"${env_file}" <<EOF
# SSH Agent socket for all user session processes (including VS Code Remote)
# Generated by devcontainer-on-windows setup
SSH_AUTH_SOCK=/run/user/${user_id}/ssh-agent.socket
EOF
  chown "${username}:${username}" "${env_file}"
  chmod 644 "${env_file}"
  log_debug "Created environment.d config: ${env_file}"
