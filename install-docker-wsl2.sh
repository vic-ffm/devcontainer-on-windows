#!/usr/bin/env bash
#===============================================================================
# install-docker-wsl2.sh - Idempotent Docker Engine installer for WSL2
#
# DESCRIPTION:
#   Production-ready script to install Docker Engine on WSL2 systems.
#   Supports any Ubuntu/Debian version with automatic codename detection.
#   Fully non-interactive, idempotent, and enterprise-ready.
#
# REQUIREMENTS:
...
#-------------------------------------------------------------------------------
# Bash Version Check & Strict Mode
#-------------------------------------------------------------------------------
...
# Fallback codenames for forward compatibility (newest first)
...
declare -a CLEANUP_ACTIONS=()       # Stack of cleanup actions to perform
declare -A CREATED_FILES=()         # Files created that may need removal
declare -A MODIFIED_FILES=()        # Files modified with backup paths
...
#-------------------------------------------------------------------------------
# Logging Functions (using EPOCHSECONDS for performance)
#-------------------------------------------------------------------------------
_log() {
    local -r level="$1" color="$2" msg="$3"
    local timestamp
    # Use EPOCHSECONDS + printf for fast timestamp (no fork)
...
# Retry with exponential backoff using nameref for cleaner syntax
retry_with_backoff() {
...
# Check if command exists (faster than 'command -v' in subshell)
has_command() {
    command -v "$1" &>/dev/null
}

# Check if running in WSL2 using regex matching
is_wsl2() {
    local version_info
...
#-------------------------------------------------------------------------------
# Lock Management (using flock - kernel-level atomic locking)
# Reference: https://mywiki.wooledge.org/BashFAQ/045
#-------------------------------------------------------------------------------
acquire_lock() {
    ...
    # Open lock file on auto-assigned FD (Bash 4.1+ feature)
...
# Allows registration of cleanup actions that will be executed in LIFO order
#-------------------------------------------------------------------------------
register_cleanup() {
    local -r action="$1"
...
# Exit code = 128 + signal_number (POSIX convention)
...
declare -rA SIGNAL_INFO=(
    ...
    # Program error signals (hardware/software faults)
    ...
    # IOT is alias for ABRT on Linux
    [IOT]="6:IOT trap:fatal"
)

# SIGEMT is not available on x86_64 Linux, only on Alpha/SPARC/MIPS
# We'll handle it gracefully if the trap fails

#-------------------------------------------------------------------------------
# Comprehensive Signal Handler
# Handles all trappable signals with appropriate cleanup and exit codes
#-------------------------------------------------------------------------------
signal_handler() {
...
cleanup() {
    ...
    for ((i = ${#CLEANUP_ACTIONS[@]} - 1; i >= 0; i--)); do
        ...
        # Run cleanup action, ignore errors (best effort)
...
# Traps all relevant signals for graceful handling
#-------------------------------------------------------------------------------
setup_signal_handlers() {
    ...
    # Linux-specific signals (may not exist on all systems)
    trap 'signal_handler STKFLT' STKFLT 2>/dev/null || true  # 16 - Stack fault

    # SIGEMT is not available on x86_64 Linux (only Alpha/SPARC/MIPS)
    # Attempt to trap it but ignore failure
    trap 'signal_handler EMT' EMT 2>/dev/null || true

    # SIGIOT is typically an alias for SIGABRT on Linux
    # Some systems may have it separately
...
resolve_codename() {
    ...
    # Use mapfile to read fallbacks into array (Bash 4.4+)
    local -a fallbacks
...
setup_docker_repository() {
    ...
    if [[ $DRY_RUN == true ]]; then
        ...
        # Register for rollback if script fails later
        register_created_file "$keyring_file"
    fi

    # Resolve effective codename (with fallback for new releases)
